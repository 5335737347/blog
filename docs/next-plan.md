...
2. 浏览器关键流程测试：现有 `smoke:prod` 已覆盖页面渲染、导航、主题、音乐、目录、
   代码块、移动端交互，以及用户登录、注册验证码全链路、忘记密码重置与重新登录、
   留言提交、后台文章创建/编辑/删除、管理员登录 redirect 和未登录访问 `/admin`
   的跳转。后续新增页面或表单时，应同步补一条冒烟断言，避免流程再次只被静态检查覆盖。
