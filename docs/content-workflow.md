...
| `project` | 项目；只匹配后台已建项目（按名称或 slug），不存在时发布报 400 |
| `coverImage` | 站内或绝对封面 URL；省略时自动从内置封面池随机初始化（覆盖更新时未提供则保留原封面） |
...
`category` 只会复用已有分类：先按显示名精确匹配，再按 slug 匹配；即使 frontmatter
中的名字与已有分类不同，也只复用而不重命名，避免一条笔记悄悄改掉全部旧文章共享的
分类名。重命名请在后台「分类管理」中显式操作。
...
`published`。发布 API 按 slug **覆盖更新**：同 slug 的笔记再次发布时会更新已有文章
（标题、正文、摘要、标签跟随笔记；slug 与公开链接不变；分类/项目/封面在笔记未提供
时保留原值；发布状态与发布日期在未显式给出时保持原样）。响应里的 `updated` 字段
标识本次是更新（true）还是新建（false）。后台批量导入不适用该规则——slug 冲突时
该文件导入失败，不会覆盖已有文章。

未提供封面的文章会自动从内置封面池随机初始化一张，之后重新发布不会更换。
