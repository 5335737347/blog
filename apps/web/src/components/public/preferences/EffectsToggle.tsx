...
export default function EffectsToggle() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div id="effects-menu" role="menu" className="absolute left-1/2 top-full mt-2 w-28 -translate-x-1/2 rounded-2xl border-2 border-pink-200 bg-white p-1.5 shadow-lg shadow-pink-100/50 dark:border-purple-800/50 dark:bg-purple-950 dark:shadow-purple-900/30 z-50">
          {options.map((opt) => (
            <button
              key={opt.value}
              role="menuitemradio"
              aria-checked={effect === opt.value}
              onClick={() => {
                setEffect(opt.value);
                setOpen(false);
              }}
