...
export default function SearchBox() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(`/api/articles?limit=8&q=${encodeURIComponent(query)}`, {
    ...
    <div ref={ref} className="relative">
      ...
      {open && visibleResults.length > 0 && (
        <div
          ...
          role="listbox"
          className="absolute left-0 top-full z-50 mt-1 w-full rounded-xl border-2 border-pink-200 bg-white shadow-lg dark:border-purple-800 dark:bg-purple-950"
        >
